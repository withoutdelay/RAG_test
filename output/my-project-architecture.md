...
- 后端：Python 服务、解析链路、检索与组装服务
- 前端：Next.js 16、React 19、TypeScript、`shadcn/ui`、Tailwind CSS v4
- 数据层：文档结构元数据、向量索引与块/资产 metadata

## 13. 数据库设计

本轮不引入新的业务数据库类型，但明确要求索引记录补齐以下结构化字段：

- `source_section_id`
- `section_path`
- `source_heading`
- 章节级 `section_retrieval_text`
- block 级 `contextualized_block_text`

## 14. API 设计

本轮保持现有 API 边界不扩张，重点是补齐返回字段与 metadata：

- 文档解析/入库链路返回 `section_catalog`
- `reuse_first` 章节生成详情返回 `selected_sections`
- `reuse_first` 章节生成详情返回 `selected_blocks`
- `reuse_first` 章节生成详情返回 `selection_reason` 与 `token_budget`

## 15. 架构选型取舍与证据链

- 放弃纯 chunk-first：章节命中正确率不足
- 放弃纯 full-section：预算和泛化标题冲突不可控
- 保留 section-first + block evidence：同时满足命中准确度与上下文密度

## 16. 架构决策账本

| 决策项 | 结论 | 影响 |
|---|---|---|
| 真值层 | `section_catalog` | 解析、索引、资产回挂统一锚点 |
| 检索主阶段 | `retrieve_sections` | 标题 exact/alias/family 成为主信号 |
| 二阶段选择 | `retrieve_blocks` | block 不再与章节候选并列竞争 |
| 装配模式 | `section_pack` / `full_section` | 以预算和置信度做运行时切换 |

## 17. Agent 执行流水线（全端）

`Document Shortlist -> Section Shortlist -> Block Selection -> Asset Backfill -> Prompt Assembly -> Trace Review -> Frontend Audit`

## 18. ADR / 数据流 / 扩展说明

- ADR：本轮核心 ADR 是“章节优先于裸 chunk”
- 数据流：解析 -> 索引 -> 章节召回 -> 章节内 block 选择 -> 资产回填 -> prompt assembly
- 扩展：后续可继续加入术语挖掘、行业 alias 扩展和更细的评估集
