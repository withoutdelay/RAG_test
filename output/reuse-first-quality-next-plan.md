...
- 批次 E：已完成
- 批次 F：已完成
- 批次 D：进行中（section-mode 真实开关验证 + warning 收敛小批次已完成）
...
状态：`进行中（第三阶段 + section-mode 真实开关验证已完成，默认仍关闭）`
...
1. 保持批次 A / B / C / E 为已完成，不再重复投入
2. 批次 F 已通过 LCI demo 端到端导出验收，不再作为当前阻断项
3. 批次 D 已进入第一阶段；下一步可继续做全文终审链路接入评估，或单独开“参数补全 / 表格物化 / warning 收敛”小批次

如果只能选 1 项，优先做全文终审链路接入评估。  
如果只能选 2 项，做全文终审链路评估 + warning 收敛。  
如果准备开一个完整小迭代，就做全文终审链路评估 + 参数补全 / 表格物化。  
`Prompt XML 化 + 清洗规则增强` 与 `VAL103` 收敛已经落地，不再列为后续近端主任务。
