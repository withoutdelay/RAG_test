...
DEFAULT_PRODUCT_FAMILIES: list[dict] = [
    {
        "code": "lci_sync_drive",
        "name": "同步电机软起动",
...
DEFAULT_PRODUCT_COMPATIBILITY: list[dict] = [
    {
...
DEFAULT_PRODUCT_CATALOG: list[dict] = [
    ...
    },
    {
        ...
        "series_name": "高压固态软起动系统",
        "code": "hv_solid_state_starter",
        "vendor": "标准产品",
        "description": "适用于高压电机限流启动、压缩机软起和短时降压旁路场景。",
        "voltage_levels": ["6kV", "10kV"],
        "min_power_kw": 500,
        "max_power_kw": 8000,
        "topology": "晶闸管降压软起",
        "applicable_motors": ["异步电机", "同步电机"],
        "applicable_loads": ["压缩机", "风机", "泵"],
        "communication_protocols": ["硬接点", "RS485"],
        "io_allocation": {"DI": 8, "DO": 6, "AI": 0, "AO": 0},
        "protection_features": ["起动超时", "过流", "过热", "缺相", "门联锁"],
        "preferred_scenarios": ["限流起动", "压缩机软起", "短时旁路切换"],
        "default_chapters": [
            "总体方案",
            "降压软起控制流程",
            "旁路与联锁方案",
        ...
        "standard_configs": [
            {
                "config_name": "标准配置",
                "components": [],
                "applicable_scenarios": ["高压固态软起"],
                "description": "包含固态软起主柜及基础联锁接口。",
            }
        ],
        "constraints": [
            {"constraint_type": "control", "condition": "需要自动旁路", "action": "需明确旁路合闸时序与联锁条件。", "severity": "warning"},
            {"constraint_type": "electrical", "condition": "同步电机", "action": "需补充同步运行侧的切换边界说明。", "severity": "warning"},
        ],
    },
    {
        "role_type": "primary",
        ...
        "series_name": "自耦变降压软起动系统",
        "code": "autotransformer_starter",
        "vendor": "标准产品",
        "description": "适用于大功率电机降压起动、调补补偿与旁路切换场景。",
        "voltage_levels": ["6kV", "10kV"],
        "min_power_kw": 2000,
        "max_power_kw": 20000,
        "topology": "调补降压自耦",
        "applicable_motors": ["异步电机", "同步电机"],
        "applicable_loads": ["压缩机", "风机", "泵"],
        "communication_protocols": ["Modbus RTU", "RS485"],
        "io_allocation": {"DI": 8, "DO": 8, "AI": 2, "AO": 0},
        "protection_features": ["起动超时", "欠压", "过流", "联锁保护"],
        "preferred_scenarios": ["降压起动", "大惯量负载", "短路容量受限母线"],
        "default_chapters": [
            "总体方案",
            "主回路与调补方案",
            "控制与旁路切换逻辑",
        ...
        "standard_configs": [
            {
                "config_name": "标准配置",
                "components": [],
                "applicable_scenarios": ["自耦降压起动"],
                "description": "包含调补变压器、控制柜与基础联锁方案。",
            }
        ],
        "constraints": [
            {"constraint_type": "electrical", "condition": "母线短路容量不足", "action": "应校核调补容量与起动压降。", "severity": "blocking"},
            {"constraint_type": "control", "condition": "多台电机共用参数", "action": "需显式定义参数切换与旁路模式。", "severity": "warning"},
        ],
    },
    {
        "role_type": "primary",
        ...
        "code": "liquid_resistor_starter",
        "vendor": "标准产品",
        "description": "适用于高压电机液阻限流起动、平滑切换和柜间联锁场景。",
        "voltage_levels": ["3kV", "6kV", "10kV", "12kV"],
        "min_power_kw": 200,
        "max_power_kw": 20000,
        "topology": "液态电阻调阻",
        "applicable_motors": ["同步电机", "异步电机"],
        "applicable_loads": ["磨机", "泵", "压缩机"],
        "communication_protocols": ["硬接点", "RS485"],
        "io_allocation": {"DI": 8, "DO": 4, "AI": 0, "AO": 0},
        "protection_features": ["起动超时", "热过载", "接地", "门锁"],
        "preferred_scenarios": ["平滑软起", "重载起动", "一次起动成功率优先"],
        "default_chapters": [
            "总体方案",
            "液阻软起原理",
            "联锁与上位机接口",
        ...
        "standard_configs": [
            {
                "config_name": "标准配置",
                "components": [],
                "applicable_scenarios": ["液阻软起"],
                "description": "包含液阻主柜、传动机构与控制部分。",
            }
        ],
        "constraints": [
            {"constraint_type": "control", "condition": "与高压柜联锁运行", "action": "需明确允许起动、运行、超时报警、综合故障信号。", "severity": "warning"},
            {"constraint_type": "environmental", "condition": "液阻介质维护周期未确认", "action": "应补充运行维护要求。", "severity": "warning"},
        ],
    },
    {
        "role_type": "support",
...
DEFAULT_PRODUCT_MODELS: list[dict] = [
    {
...
DEFAULT_PRODUCT_INTERFACES: list[dict] = [
    {
        "series_code": "lci_sync_drive",
        ...
        "notes": "来自湛江中纸 LCI 主样本，现场总线适配器暂定 Profibus-DP，本地 PLC 联控 LCI 与励磁柜。",
    },
    {
        "series_code": "lci_sync_drive",
