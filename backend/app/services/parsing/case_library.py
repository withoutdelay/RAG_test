...
def build_outline_library_entry(
    *,
    ...
    flattened = flatten_outline_nodes(nodes)
    return {
        ...
        "top_level_titles": [node.title for node in nodes],
        "outline_tree": [node.to_dict() for node in nodes],
    ...
    *,
    ...
    for chunk in chunks:
        if not _is_reusable_chunk(chunk):
            continue
        section_class = _suggest_section_class(chunk.heading_path or "")
